...
export const MODEL_VERSION = "v9 (debit spreads removed, GIFT Nifty scored, VIX color fixed, site password-protected — 28 Jun 2026)"
...
export const MODEL_CHANGELOG = [
  ...
  {
    version: "v6",
    factors: ["All v5 factors", "NIFTY-only trade history (SENSEX excluded per user instruction)", "Hard DTE-0 warning enforced in the strategy engine", "Sell-side-default rule", "300pt Bull Put Spread width default", "New Pattern Intelligence tab"],
    addedOn: "28 Jun 2026",
    retiredOn: "28 Jun 2026",
    reason: "User uploaded a fresh 14-month trade history (3,897 NIFTY legs, 285 combos, 16 Apr 2025 to 25 Jun 2026) and explicitly instructed: 'we are not going to trade in sensex, so removed all sensex trades and only NIFTY.' Re-running the analysis NIFTY-only sharpened several findings: DTE 0 entries are now an even more extreme outlier (20% win rate, -Rs 7.30L across 54 trades -- the single worst bucket in the whole history), every buy-side structure (Long CE, Long PE, Bull Call Spread, Bear Put Spread) lost 100% of the time across 8 trades, and 300-point-wide Bull Put Spreads won 10/10 times. These are no longer just narrated in the Playbook -- the strategy engine (lib/strategy.js) now surfaces an explicit critical-severity warning whenever a trade would be entered at DTE 0, and a new Pattern Intelligence tab shows every underlying number so the rule is never asserted without its evidence next to it.",
  },
  {
    version: "v7",
    factors: ["All v6 factors", "Corrected FII/DII cash figures (were stale)", "FII cash-vs-futures divergence check", "Pseudo-Signal Watch (4 sourced traps)", "Expiry date shown on every trade recommendation", "Holiday-status re-verification"],
    addedOn: "28 Jun 2026",
    retiredOn: "28 Jun 2026",
    reason: "User asked for deeper research, cross-verification of every data point, explicit expiry dates on trade recommendations, and a hunt for pseudo-signals that could misguide the model. Fresh research surfaced and fixed a real error: the dashboard's fallback FII/DII figures (FII -Rs 1,541.08 Cr, DII +Rs 2,715.17 Cr) were stale and did not match the verified 25 Jun figures (FII +Rs 383.76 Cr, DII +Rs 5,747.75 Cr) -- now corrected. Research also surfaced a genuine pseudo-signal: FII cash buying of Rs 384 Cr looked bullish in isolation, but FII sold Nifty futures net -2,23,809 contracts the same day, which institutional-flow trackers classify as profit-taking with downside cover, not a clean bullish signal -- this is now an explicit check, not a hidden assumption. A second pseudo-signal was confirmed: Iran's 26 Jun drone strike on a vessel in the Strait of Hormuz sounded like a fresh escalation, but crude oil still fell 4.34% the same session, meaning the market's own price reaction disagreed with the headline's apparent severity -- price reaction is now treated as the real signal, the headline as just an input. A third check resolved an internal date discrepancy: some sources implied 26 Jun was a normal trading day, but two independent sources confirmed NSE/BSE were genuinely closed for Muharram that day -- the weekend-gap dampener logic was verified correct, not just assumed. Every trade recommendation now states its actual expiry date (computed from DTE), not just the DTE number.",
  },
  {
    version: "v8",
    factors: ["All v7 factors", "lucide-react SVG icons (replaced broken tabler-icons webfont)", "Weekly forecast-verification cron (Monday 10 AM IST)", "Hard file-existence verification process for deploys"],
    addedOn: "28 Jun 2026",
    retiredOn: "28 Jun 2026",
    reason: "Two real defects were reported by Cowork after deploying v7, both now fixed. First: three of four v7 commits failed to build because lib/tradeHistory.js was never actually committed to GitHub -- it was correctly flagged as a new file in the v6 instructions, but the v7 instructions incorrectly listed it under 'only replace if different', so it was skipped. The deploy process is changed: every future instruction set will require an explicit existence check for every imported file, not a soft 'check if different' for files that were new in a prior version. Second: the tabler-icons webfont (loaded via a CDN <link> tag) was rendering empty icon-wrapper circles with no glyph inside -- this is a documented, recurring problem with that exact package across multiple browsers (GitHub tabler/tabler-icons issues #476, #1327, #1415, #1452 all describe the same symptom). Replaced entirely with lucide-react, a real bundled SVG icon library -- icons are now compiled into the page at build time and cannot fail to load the way a runtime font download can. Also added: a Monday 10 AM IST cron job (vercel.json) that pulls Friday's actual NIFTY close and compares it against the stored Friday prediction, flagging a match or miss -- documented honestly as a flag-and-store step, not a fully automated root-cause writer, since writing the specific reason a prediction failed requires reasoning about that week's actual news and data, which a cron job cannot do on its own.",
  },
  {
    version: "v9",
    factors: ["All v8 factors", "GIFT Nifty added as a real scoring factor (was missing entirely)", "VIX heatmap color inverted (falling VIX now shows green, not red)", "Debit spreads removed from the strategy engine entirely", "Sell-side ratio spread added as the profit-can-exceed-loss alternative", "MCX trade ideas (strike/strategy) added to every commodity card", "MCX backtest corrected to its real 3-day tracking window", "Prediction log and MCX backtest tables reversed to latest-first", "News & Catalyst Watch panel added", "Predicted market levels promoted to a dedicated strip directly below the direction verdict", "Site-wide HTTP Basic Auth password protection (middleware.js)"],
    addedOn: "28 Jun 2026",
    reason: "A detailed line-by-line review of the live site surfaced multiple real defects, all fixed here. (1) GIFT Nifty -- explicitly named by the user as 'our major item' -- was completely absent from the global indices list; added, and also wired into the actual composite score, not just displayed. (2) The heatmap coloured a falling India VIX red, directly beside a bullish verdict it should have been confirming, not contradicting -- VIX and other fear-gauge indices now invert color logic (falling = green = bullish). (3) MCX commodity cards showed direction and levels but never an actual strike, premium, or strategy to place -- every card now carries a tradeIdea block with sell/buy strikes and a rationale, consistent with the sell-side-only philosophy. (4) The MCX backtest table claimed weekly entries going back to 27 May, implying a month of tracking that never happened -- MCX tracking on this dashboard actually started 3 days ago; the fabricated older rows were removed rather than kept as misleading filler, and an honest scope banner now states the real window. (5) The most serious fix: despite the dashboard's own Pattern Intelligence tab showing every buy-side structure lost 100% of the time across 8 real trades, the strategy engine still generated and sometimes recommended two debit-spread variants as the 'primary' trade -- a direct contradiction between the dashboard's analysis and its own recommendation. Both debit spreads are removed; a sell-side ratio spread (sell 1, buy 2 further out) now serves the 'profit can exceed loss' need without becoming a net buyer of premium. (6) Both the prediction log and MCX backtest tables read oldest-first, opposite of how a daily-checked table should read -- both now show latest first. (7) Predicted support/resistance/max-pain levels were buried as small inline text inside the verdict card -- promoted to their own prominent metric strip directly below the direction headline, as requested. (8) A News & Catalyst Watch panel was added with real, cross-checked market news (India regaining 5th-largest market cap globally, FII selling despite a rising index, sector divergence) so breaking news is aggregated and its impact assessed in one place, not left implicit. (9) Site-wide password protection was added via Next.js Middleware (free on every Vercel plan, including Hobby) after confirming Vercel's own Password Protection feature requires either Enterprise or a $150/month Pro add-on -- not proportionate for this use case.",
  },
]
...
export function build325Strategy(direction, niftyClose, confidence) {
  ...
  return {
    ...
    rules: [
      ...
      'Exit everything by 9:20 AM the next morning regardless of P&L if the position was held overnight unintentionally — do not let an undefined-risk single-leg position run into a second full session without a fresh decision.',
    ],
  }
}
