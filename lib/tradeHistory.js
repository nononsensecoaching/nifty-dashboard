...
export const BULL_PUT_SPREAD_WIDTH_PATTERN = {
  finding: '300-point wide Bull Put Spreads won 10/10 times in this history (100%). 200-point spreads won 3 of 5 (60%). One isolated 350-point trade also won (1/1). One 100-point spread lost.',
...
export const HARD_RULES = [
  ...
  },
  {
    ...
    rule: 'Use 300-point width for Bull Put Spreads (and the equivalent on Bear Call Spreads) on NIFTY by default — never narrower than 300 without a specific reason',
    evidence: '300pt: 10/10 wins (100%). 350pt: 1/1 win (100%). 200pt: 3/5 wins (60%). One 100pt attempt: 0/1 (loss). Width is the single sharpest differentiator found in the entire trade history.',
    severity: 'critical',
  },
  {
    id: 'tranche-entry',
    rule: 'Build the position across 2-3 nearby strikes as the day develops, rather than committing full size to one exact strike pair at once',
    evidence: 'Not a single one of the 10 winning 300-point Bull Put Spreads was a clean, single-strike pair — every one was built across 2-3 sold strikes and 2-3 bought strikes intraday. This was how the winners were actually constructed, not an optional refinement.',
    severity: 'medium',
  },
  {
    id: 'hedge-outside-full-range',
    rule: 'When scaling into multiple strikes, every bought (hedge) leg must sit outside the FULL range of sold strikes — not just outside any single one of them',
    evidence: 'Both 200-point losses shared one exact mechanical flaw: the bought hedge strike landed BETWEEN two sold strikes rather than cleanly outside the whole range. On 5 Dec, puts were sold at both 25,800 and 26,200, but the hedge was bought at 26,000 — nested inside, protected from neither side. That single misplaced leg cost more than both credit legs combined.',
    severity: 'critical',
  },
  {
    id: 'exit-60-70pct-same-day',
    rule: 'Exit once 60-70% of the collected credit has decayed, or by end of day regardless — do not hold into the next session hoping for more decay',
    evidence: 'Every position in the winning 300-point sample closed the same day it was entered, with NIFTY still on the favourable side of the sold strike at exit. None were held overnight.',
    severity: 'high',
  },
]
